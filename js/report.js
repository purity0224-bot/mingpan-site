...
(function (global) {
  ...
  function contradictions(d) {
    ...
    if (shifted.length) {
      list.push({
        ...
        verdict: '兩套黃道原點不同（相差 ayanamsa ' + V.ayanamsa.toFixed(1) + '°），不代表兩者皆真或互相打架。西占讀「季節性人格光譜」、印占讀「恆星背景與業力軸」，各在各的座標系內解讀，禁止混軸。',
      });
    }
    ...
    if (!H.definedCenters.includes('薦骨') && driveP && driveP.n >= 3) {
      list.push({
        ...
        verdict: '想衝的慾望是真的，持續產能的引擎是借來的——調和方向：以「短衝刺＋長休整」的節奏取代等速輸出，把發起／指揮的位置留給自己，把執行綿延的部分外包給環境或團隊。',
      });
    }
    ...
    if (B.strength.includes('中和')) {
      list.push({
        ...
        verdict: '不同流派可能給出相反的喜用神。本報告的喜用（' + B.favorable.join('、') + '）僅視為工作假設，建議以第九節假說核對實際年份經驗後再校準。',
      });
    }
    return list;
  }
  ...
  function synthesize(d) {
    ...
    const domains = [];
    {
      ...
      let text = guanScore >= 3 ? '事業宮位強，適合走「有名有位」的路線，往能被看見的位置移動。' : guanScore <= -1 ? '事業宮帶壓，成就感建議別全押在頭銜上，用作品和專業累積替代職位競逐。' : '事業宮中性，成就高低更多取決於你選的賽道是否用得上你的核心強項。';
      if (lead && lead.n >= 3) text += '多系統同指領導力——別只做執行者，爭取主導權反而輕鬆。';
    ...
    const horizons = { short: [], mid: [], long: [] };
    {
      ...
      if (luR) horizons.short.push(`今年的機會窗在「${luR.split('化祿入')[1]}」領域——這裡送上門的邀請認真評估（依據：紫微流年化祿）`);
    ...
    {
      horizons.long.push(`約 35 歲後生活重心會往「${Z.shenPalace.name}」（${({ 命宮: '自我狀態', 兄弟: '同輩手足', 夫妻: '感情婚姻', 子女: '子女與創作', 財帛: '金錢', 疾厄: '健康', 遷移: '外出變動', 交友: '人脈圈', 官祿: '事業', 田宅: '居住資產', 福德: '心境享受', 父母: '長輩體制' })[Z.shenPalace.name] || Z.shenPalace.name}）移——長期布局把資源預留給這一塊（依據：紫微身宮）`);
  ...
  const SKILL_CARDS = {
    ...
    '美感與藝術頻道': { roles: '設計視覺、空間陳列、選物策展、攝影影像、品牌美術', collab: '團隊的審美底線——東西出門前給你過目', misuse: '只會嫌不出手、美感變成購物藉口——沒有作品，品味會退化成刻薄', step: '本季完成一件小作品（一張海報、一組照片、一個角落改造）並公開給人看' },
  };
  ...
  function actionPlan(d) {
    ...
    const career = {
      ...
      mask: `${MC_STYLE[W.mc.sign] || '穩健專業路線'}。面試與個人品牌照這個人設經營最省力（依據：天頂${W.mc.sign}）`,
    };
    ...
    const learning = {
      ...
      how: `學法：${(studyP && studyP.n >= 2) ? '一次只開一口井，設公開交付日防止無限延期' : '先接一個真實任務再補知識，讓需求拉著你學'}；學到的東西${ps.find((p) => p.dim === '表達與傳播' && p.n >= 2) ? '用你的表達優勢輸出（寫、講、教），記憶會加倍固化' : '做成自己的 checklist 或筆記系統，比重看教材有效'}。`,
    ...
    const yearCal = at.rows.slice(0, 3).map((r) => {
      const extra = [];
      if (r.reasons.some((x) => x.includes('太歲') && !x.includes('合'))) extra.push('犯太歲年：儀式性大事（婚嫁、購屋）多做功課、保持低調彈性');
    ...
    const decisions = [
      ...
      },
      {
        q: '要不要搬家／換城市？',
        steps: [
          `【體質判斷】${B.shensha.some((s) => s.name === '驛馬') ? '你帶驛馬——移動對你是加分項，想搬通常值得認真評估' : '你的盤不特別驛馬——搬家前先確認是「想去那裡」還是「想逃這裡」'}`,
  ...
  function strategies(d) {
    ...
    const PLAYBOOK = {
      ...
      '情感深度與感受力': '感受力不是要修掉的 bug：給它獨立頻寬（寫、獨處、藝術），不要讓它只能從最近的人身上找出口。',
      ...
      '研究、深度與洞察': '深度需要圍欄：一次只開一口井，並公開announce你在挖什麼——防止深度變成無限延期的藉口。',
    ...
    out.push({
      ...
      move: H.authority.includes('情緒') ? '所有重大決定過一夜再簽：你的清明不在當下，在波形走完之後。' :
